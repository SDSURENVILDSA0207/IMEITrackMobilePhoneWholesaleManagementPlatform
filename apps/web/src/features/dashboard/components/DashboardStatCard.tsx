...
const accentStyles: Record<Accent, { ring: string; iconBg: string; icon: ReactNode }> = {
  indigo: {
...
export function DashboardStatCard({ label, value, accent = "slate", loading }: DashboardStatCardProps) {
  ...
  return (
    <div
      className={`group relative overflow-hidden rounded-2xl border border-slate-200/70 bg-white/95 p-5 shadow-soft ring-1 transition duration-150 hover:-translate-y-[1px] hover:shadow-raised ${a.ring}`}
    >
      <div className="pointer-events-none absolute -right-4 -top-8 h-20 w-20 rounded-full bg-slate-100/70 blur-2xl transition group-hover:scale-110" />
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-500">{label}</p>
          {loading ? (
            <div className="mt-3 h-9 w-24 animate-pulse rounded-md bg-slate-100" aria-hidden />
          ) : (
            <p className="mt-2 text-3xl font-semibold tabular-nums tracking-tight text-slate-900">{value}</p>
          )}
        </div>
        <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ring-1 ring-inset ring-white/60 ${a.iconBg}`}>{a.icon}</div>
      </div>
    </div>
  );
}
